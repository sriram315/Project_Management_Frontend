...
export const projectAPI = {
  ...
  // Create new project
  create: async (projectData: CreateProjectData): Promise<{ id: number; message: string }> => {
    const response = await fetch(`${API_BASE_URL}/projects`, {
      method: 'POST',
      headers: {
  ...
  // Update project
  update: async (id: number, projectData: Partial<CreateProjectData>): Promise<{ message: string }> => {
...
export const projectTeamAPI = {
  ...
  getAvailableTeam: async (projectId: number): Promise<any[]> => {
    const response = await fetch(`${API_BASE_URL}/projects/${projectId}/available-team`);
    if (!response.ok) {
  ...
  addTeamMember: async (projectId: number, userId: number, allocatedHoursPerWeek: number, username?: string, role?: string): Promise<{ message: string }> => {
    const response = await fetch(`${API_BASE_URL}/projects/${projectId}/team`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
  ...
  removeTeamMember: async (projectId: number, teamMemberId: number): Promise<{ message: string }> => {
  ...
  updateTeamMember: async (projectId: number, userId: number, allocatedHoursPerWeek: number): Promise<{ message: string }> => {
...
export const taskAPI = {
  ...
  // Create new task
  create: async (taskData: CreateTaskData): Promise<{ id: number; message: string }> => {
    const response = await fetch(`${API_BASE_URL}/tasks`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
  ...
  // Update task
  update: async (id: number, taskData: UpdateTaskData): Promise<{ message: string }> => {
...
export const authAPI = {
  // Login
  login: async (username: string, password: string): Promise<{ id: number; username: string; role: string }> => {
...
export const userAPI = {
  ...
  getUserProjectDetails: async (userId: number, projectId: number): Promise<any> => {
    const response = await fetch(`${API_BASE_URL}/users/${userId}/projects/${projectId}`);
    if (!response.ok) {
  ...
  getUserProjectTasks: async (userId: number, projectId: number): Promise<Task[]> => {
    const response = await fetch(`${API_BASE_URL}/users/${userId}/projects/${projectId}/tasks`);
    if (!response.ok) {
      throw new Error('Failed to fetch project tasks');
    }
    return response.json();
...
export const dashboardAPI = {
  ...
  getTaskStatus: async (filters?: { projectId?: string; employeeId?: string; startDate?: string; endDate?: string }) => {
  ...
  getTasksTimeline: async (params: { role: string; userId: number; projectId?: string; employeeId?: string }) => {
    const query = new URLSearchParams();
    query.append('role', params.role);
    query.append('userId', String(params.userId));
    if (params.projectId) query.append('projectId', params.projectId);
    if (params.employeeId) query.append('employeeId', params.employeeId);
    const response = await fetch(`${API_BASE_URL}/dashboard/tasks-timeline?${query.toString()}`);
